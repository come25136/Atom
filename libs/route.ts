...
let translations: Map<string, Inames>,
  stops: Map<string, _Istop>,
  stopTimes: Map<string, { [key: string]: IstopTimes[] }>;

export default (line: number | string, _date: string) =>
  new Promise<Istop[]>(async (resolve, reject) => {
    ...
    resolve(
      routes[time].map(stop_raw => {
        ...
        return {
          ...
          name: Object.assign(
            { ja: "", "ja-Hrkt": "", en: "" },
            translations.get(stop.stop_name)
          ),
