...
export class TransferRepository extends BaseRepository<Transfer> {
  ...
  async findOneByRemoteUidAndFronStopIdAndToStopId(remoteUid: Remote['uid'], fromStopId: Transfer['fromStopId'], toStopId: Transfer['toStopId'], other?: FindOneOptions<Transfer>): Promise<Transfer> {
    return this.findOne({
      ...other,
      where: {
        fromStopId,
